...
if [ -b /run/initramfs/p_pt ] && [ ! -h /run/initramfs/ro_ovl ]; then
    ...
    read -r ovltmpfsopts > /dev/null 2>&1 < /run/initramfs/ovltmpfsopts
    mount -m -t tmpfs os_tmp -o mode=0755${ovltmpfsopts:+,"$ovltmpfsopts"} /run/ovl
