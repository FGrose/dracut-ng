#!/bin/sh
# overlayfs-lib.sh: utilities for OverlayFS use

command -v getarg > /dev/null || . /lib/dracut-lib.sh
command -v parse_cfgArgs > /dev/null || . /lib/partition-lib.sh

# Fetch non-boolean values for rd.overlay or fall back to rd.live.overlay
# $1 - $OverlayFS - OverlayFS mount source name default.
get_rd_overlay() {
    rd_dm_overlay="$(getarg rd.dm.overlay -d rd.live.overlay)" \
        && IFS=, parse_cfgArgs ovl,"$rd_dm_overlay"
    OverlayFS="$(getarg rd.live.overlay.overlayfs)" && {
        warn "Kernel command line option 'rd.live.overlay.overlayfs' is deprecated, use 'rd.overlay' instead."
        case "$OverlayFS" in
            0 | no | off) unset -v 'OverlayFS' ;;
            '' | 1)
                [ "$rd_dm_overlay" ] || {
                    # For legacy case of using dmsquash-live for an OverlayFS
                    #   on a regular block root device:
                    OverlayFS=os_rootfs
                    warn "This use of 'rd.live.overlay.overlayfs' is deprecated, use 'root=ovl:<device_specification>' instead."
                }
                OverlayFS="$1"
                ;;
        esac
    }
    volatile=volatile
    # Set p_pt, p_ptfstype, size; toggle volatile.
    rd_overlay="$(getarg rd.overlay)" && IFS=, parse_cfgArgs ovl,"$rd_overlay"
    : "${OverlayFS:="${1-os_rootfs}"}"
    ovl_dir="$(getarg rd.ovl.dir -d rd.live.dir)"
    case "$1" in
        os_rootfs) : "${ovl_dir:=RootOvl}" ;;
        LiveOS_rootfs) : "${ovl_dir:=LiveOS}" ;;
    esac
    [ "$OverlayFS" ] && {
        load_fstype overlay || {
            unset -v 'OverlayFS'
            rm /run/initramfs/p_pt
            ETC_KERNEL_CMDLINE="$ETC_KERNEL_CMDLINE rd.overlay=0"
        }
    }
    echo "$OverlayFS" > /run/initramfs/OverlayFS
    : "${p_pt_lbl:="${OverlayFS%_rootfs}"_persist}"
    strstr "$rd_overlay" PROMPT && {
        unset -v 'volatile'
        p_pt=/dev/disk/by-label/"$p_pt_lbl"
    }
    [ "$SYSTEMD_IN_INITRD" = 1 ] && [ "$p_pt" ] && echo "$p_pt" > /run/systemd/generator.early/p_pt
    [ "$volatile" ] || [ "${p_ptFlags+set}" ] || set_FS_options "$p_ptfsType" p_ptFlags
    getargbool 0 rd.overlay.reset -d -y rd.live.overlay.reset && ln -sf yes /run/initramfs/reset_ovl
    getargbool 0 rd.overlay.readonly -d -y rd.live.overlay.readonly && {
        readonly_overlay=--readonly
        volatile=volatile
    }
}

# Called from overlayfs-generator.sh or dmsquash-generator.sh
overlayfs_mount_generator() {
    local p_pt_unit_name _OverlayFS
    read -r p_pt_unit_name < /run/systemd/generator.early/p_pt
    [ "$p_pt_unit_name" ] && {
        p_pt_unit_name="$(dev_unit_name "$p_pt_unit_name")"
        set_FS_options "$p_ptfsType" p_ptFlags

        echo "# Automatically generated by ${0##*/} overlayfs_mount_generator()
[Unit]
DefaultDependencies=no
Conflicts=umount.target
After=sysinit.target
Before=dracut-pre-mount.service
Requires=systemd-fsck@$p_pt_unit_name.service
After=systemd-fsck@$p_pt_unit_name.service
[Mount]
Where=/run/$p_pt_lbl
What=/run/initramfs/p_pt
Options=defaults,noauto,$p_ptFlags
Type=${p_ptfsType-auto}" > "$GENERATOR_DIR"/run-"$p_pt_lbl".mount

        mkdir -p "$GENERATOR_DIR"/run-"$p_pt_lbl".device.d
        echo "[Unit]
JobTimeoutSec=3000
JobRunningTimeoutSec=3000" > "$GENERATOR_DIR"/run-"$p_pt_lbl".device.d/timeout.conf
    }

    echo "# Automatically generated by ${0##*/} overlayfs_mount_generator()
[Unit]
DefaultDependencies=no
[Mount]
Where=/run/rootfsbase
What=/run/initramfs/rorootfs
Options=defaults,noauto,$rflags,ro
Type=${rfstype-auto}" > "$GENERATOR_DIR"/run-rootfsbase.mount

    mkdir -p "$GENERATOR_DIR"/run-rootfsbase.device.d
    echo "[Unit]
JobTimeoutSec=3000
JobRunningTimeoutSec=3000" > "$GENERATOR_DIR"/run-rootfsbase.device.d/timeout.conf

    basedirs=lowerdir="${readonly_overlay:+/run/overlayfs-r:}"/run/rootfsbase
    echo "# Automatically generated by ${0##*/} overlayfs_mount_generator()
[Unit]
DefaultDependencies=no
After=dracut-pre-mount.service
Before=initrd-root-fs.target
[Mount]
Where=/sysroot
What=${OverlayFS:=os_rootfs}
Options=${volatile:+volatile,}${basedirs},upperdir=/run/overlayfs,workdir=/run/ovlwork
Type=overlay" > "$GENERATOR_DIR"/sysroot.mount

    _OverlayFS=$(echo "$OverlayFS" | sed 's,/,\\x2f,g;s, ,\\x20,g;s,-,\\x2d,g;')

    echo "[Unit]
JobTimeoutSec=3000
JobRunningTimeoutSec=3000" > "$GENERATOR_DIR/$_OverlayFS".device.d/timeout.conf
}

get_ovlpath() {
    [ -b /run/initramfs/p_pt ] && {
        read -r ovlpath < /run/initramfs/ovlpath
        [ "$ovlpath" = auto ] && unset -v 'ovlpath'
        : "${ovlpath:=/"$ovl_dir"/overlay-"$label"-"$uuid"}"
        str_starts "$ovlpath" '/' || ovlpath=/"$ovlpath"
    }
}

# $1 - mntDir
do_overlayfs() {
    [ "$ovlpath" ] || get_ovlpath
    mkdir -m 0755 -p "${mntDir:=/run/"${OverlayFS%_rootfs}"_persist}"
    local mount_unit=/run/systemd/generator.early/run-"${mntDir##*/}".mount
    p_pt=$(readlink -f "$p_pt")
    # Get last mountpoint for $p_pt.
    get_MP_PT : mp "$p_pt"
    if [ "$MP" ]; then
        # We need $p_pt writable for overlay storage
        [ ! -w "$mntDir" ] && [ ! "$readonly_overlay" ] && mount -o remount,rw "$mntDir"
    else
        [ "$p_ptFlags" ] || set_FS_options "${p_ptfsType:=$(det_fs "$p_pt")}" p_ptFlags
        ln -sf "$p_pt" /run/initramfs/p_pt
        [ -f "$mount_unit" ] && {
            echo "Options=$p_ptFlags
Type=${p_ptfsType:-auto}" >> "$mount_unit"
        }
        [ "$p_ptFlags" ] || {
            det_fs "$p_pt"
            set_FS_options "${p_ptfsType:=$FS}" p_ptFlags
        }
        ln -sf "$p_pt" /run/initramfs/p_pt
        fstype="${p_ptfsType:-auto}" srcPartition="$p_pt" \
            mountPoint="$mntDir" srcflags="$p_ptFlags" fsckoptions="$fsckoptions" \
            mount_partition
    fi
    if ismounted "$mntDir"; then
        mkdir -p "$mntDir${ovl_dir:+/"$ovl_dir"}/ovlwork" "$mntDir$ovlpath"
    else
        prompt_message \
            '   Failed to mount the persistent overlay; using a temporary one.' \
            '  All new root filesystem changes will be lost on shutdown.' \
            '  Press [Enter] to continue.'
    fi
    # Establish links needed for the OverlayFS mount.
    ln -s "$mntDir$ovlpath" /run/overlayfs${readonly_overlay:+-r}
    [ "$readonly_overlay" ] || ln -s "$mntDir$ovlpath"/../ovlwork /run/ovlwork
    setup=OverlayFS_setup

    if [ "$readonly_overlay" ]; then
        if [ "$setup" ]; then
            info "Using a temporary overlay."
        elif [ "$p_pt" ] && [ "$ovlpath" ]; then
            prompt_message \
                '   Unable to find a persistent overlay; using a temporary one.' \
                '  All root filesystem changes will be lost on shutdown.' \
                '  Press [Enter] to continue.'
        fi
        if [ "${OverlayFS}$rd_overlay" ] && ! [ -h /run/overlayfs-r ]; then
            prompt_message \
                '   Failed to find a persistent overlay; using a temporary one.' \
                '  All root filesystem changes will be lost on shutdown.' \
                '  Press [Enter] to continue.'
            unset -v 'readonly_overlay'
            ETC_KERNEL_CMDLINE="$ETC_KERNEL_CMDLINE rd.overlay.readonly=0"
        fi
        [ "$ETC_KERNEL_CMDLINE" ] && {
            mkdir -p /etc/kernel
            printf '%s' " $ETC_KERNEL_CMDLINE" >> /etc/kernel/cmdline
            [ "${DRACUT_SYSTEMD-}" ] && systemctl daemon-reload
        }
    fi
}
