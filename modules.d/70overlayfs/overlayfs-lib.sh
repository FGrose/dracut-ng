...
set_FS_options() {
    ...
    case "$p_ptfsType" in
        btrfs)
            p_ptFlags=${p_ptFlags:+${p_ptFlags},}'compress=zstd:3'
            ;;
        f2fs)
            strstr "${extra_attrs:=extra_attr,inode_checksum,sb_checksum,compression}" compression \
                && ${p_ptFlags:+${p_ptFlags},}='compress_algorithm=zstd:6,compress_chksum,atgc,gc_merge'
            ;;
    esac
    local rd_ovl_flags
    rd_ovl_flags=$(getarg rd.ovl.flags)
    if [ "$rd_ovl_flags" ]; then
        p_ptFlags=${p_ptFlags:+${p_ptFlags},}"$rd_ovl_flags"
    else
...
mount_p_Partition() {
    ...
    if [ "${DRACUT_SYSTEMD-}" ]; then
         systemctl start run-os_persist.mount
    else
        # Repurpose 99-mount-root.sh for the persistence partition.
        mntcmd="$hookdir"/mount/99-mount-root.sh
        fstype="${p_ptfsType:-auto}" srcPartition="$overlay_pt" mountPoint="$mntDir" srcflags="$p_ptFlags" "$mntcmd" override
    fi
...
do_overlayfs() {
    ...
    if [ "$overlay_pt" ] && [ "$ovlpath" ]; then
        ...
        if [ "$1" ]; then
            ...
            set_FS_options "${p_ptfsType:=$(blkid --probe --match-tag TYPE --output value --usages filesystem "$overlay_pt")}"            
            mount_p_Partition
        fi
