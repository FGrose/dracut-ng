...
`'}
    ...
    warnx="$3"
    set +x
    local d_path s_path type sectors size_gb whole tenth blk_info label model serial n_path
    discs=$(
        set -x
        printf '%-15s %-8s %-28s %11s %-20s %-20s %-5s\n' \
            "PATH" "FSTYPE" "LABEL" "SIZE_GiB " "MODEL" "SERIAL" "TYPE"
        ...
        for d_path in /dev/sd[a-z]${d:+*} /dev/nvme[0-9]n[0-9]${d:+*} /dev/mmcblk[0-9]${d:+*}; do
            ...
            serial=''
            [ "$type" = part ] || {
                [ -f "$n_path"/device/model ] && read -r model < "$n_path"/device/model

                # Try standard SCSI/SATA/NVMe serial files
                if [ -f "$n_path"/device/serial ]; then
                    read -r serial < "$n_path"/device/serial
                elif [ -f "$n_path"/device/device/serial ]; then
                    read -r serial < "$n_path"/device/device/serial
                elif [ -f "$n_path"/uevent ]; then
                    while read -r line; do
                        case "$line" in
                ...
                [ "$serial" ] || serial=$(udevadm info -q property --value --property=ID_SERIAL_SHORT $n_path)
            }
            printf '%-15s %-8s %-28s %11s %-20s %-20s %-5s\n' \
                "$d_path" "${fstype:-}" "${label:-}" "$size_gb " "${model:-}" "${serial:-}" "$type"
        done
    )
...
\`"
    ...
    end_block() {
        ...
        echo "$REPLY" > /run/initramfs/live_dir
    }
    prompt_for_input
}
...
prompt_for_size() {
    local - OLDIFS space _warn sz_max dev s_path l_path d_node sz w t blk_info model fstype partlabel
    set +x
    ...
    space=$(
        ...
        for s_path in /sys/class/block/${dev}*; do
            ...
            model=''
            l_path="$s_path"
            [ -f "$s_path"/partition ] && l_path="${s_path%/*}"
            [ -f "$l_path"/device/model ] && read -r model < "$l_path"/device/model
            ...
            printf '%-14s %-16s %-14s %-28s %-8s %9s\n' \
                "$d_node" "${model:-}" "${partlabel:-}" "${label:-}" "${fstype:-}" "$sz"
        done
    )
    OLDIFS="$IFS"
    IFS='
...
'
    ...
    [ "$espStart" ] && {
        ...
        [ -d /run/initramfs/isoscan ] && read -r isofile < /run/initramfs/isofile
    }
