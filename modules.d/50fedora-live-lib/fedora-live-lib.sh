...
update_BootConfig() {
    local _live_dir root rootcfg livedev ovl_spec ovl _TITLE GRUB_cfg cfg PARTUUID ovl_puuid sedcmd label
    read -r cfg < /run/initramfs/cfg
    cfg="${cfg%%[:,+]*}"
    if [ "$mntDir" ]; then
        # OverlayFS present.
        OverlayFS=rd\.live\.overlay\.overlayfs=LiveOS_rootfs
        root=/run/rootfsbase
        livedev=$mntDir
    else
        root=$NEWROOT
    ...
    set -- $(findmnt -nro SOURCE,PARTUUID "$livedev")
    livedev=$1
    PARTUUID=$2
...
s/\s+(quiet|rhgb|splash)\s+(quiet|rhgb|splash)\s+/ /
    ...
    rootcfg=PARTUUID=$PARTUUID
    ovl_spec="${PARTUUID}${ovl_spec:+:$ovl_spec}"
    ovl_puuid=$PARTUUID
...
\        initrds=\"\$initrds \$f\"\\
\    done
               s/root=live:CDLABEL=\S+/root=live:$rootcfg rw${ovl_spec:+ rd\.live\.overlay=PARTUUID=$ovl_spec} $cfgargs/
               }
               /^\s+linux|initrd/ {
...
...
    ...
    sed -i -r "s/(\s+rd.live.(overlay|image)=PARTUUID=)[-a-fA-F0-9]+( |,\S+ )/\1${ovl_puuid}\3/
