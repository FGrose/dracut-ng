...
[ "${mntcmd+mount}" ] && {
    ...
    sleep 0.1
    if [ "${DRACUT_SYSTEMD-}" ]; then
        mount_unit=/run/systemd/generator.early/run-initramfs-live.mount
        echo "Type=$livedev_fstype" >> "$mount_unit"
        systemctl start run-initramfs-live.mount
    else
...
[ "$espStart$base_dir$cfg" ] && {
    ...
    set -- /run/initramfs/ESP/*/boot /run/initramfs/ESP/*/images
    [ -e "$1" ] || [ -e "$2" ] && {
...
do_live_overlay() {
    ...
    if [ ! "$setup" ] && [ "$p_pt" ]; then
        ...
        # Get last mountpoint for $p_pt.
        get_MP_PT : mp "$p_pt"
        if [ "$MP" ]; then
            [ "$MP" = "$mntDir" ] || mount --bind "$MP" "$mntDir"
            ...
            [ "${p_ptFlags+set}" ] || {
                det_fs "$p_pt"
                set_FS_options "${p_ptfsType:=$p_pt}" p_ptFlags
            [ -f "$mount_unit" ] && {
...
Type=${p_ptfsType:-auto}" >> "$mount_unit"
        ...
        if [ -f "$mntDir$ovlpath" ] && [ -w "$mntDir$ovlpath" ]; then
            ...
            case $FS in
                # (Uninitialized DM_snapshot_cow returns 'auto'.)
    ...
    [ "$OverlayFS" ] || {
        if [ "$readonly_overlay" ] && [ "$OVERLAY_LOOPDEV" ]; then
            echo 0 "$sz" snapshot "$loopdev" "$OVERLAY_LOOPDEV" P 8 | dmsetup create --readonly live-ro
        ...
        echo 0 "$sz" linear "$loopdev" 0 | dmsetup create --readonly live-base
    }
    ln -s "$FSIMG" /dev/live-base
}
# end do_live_overlay()
...
[ "$ro" ] || {
    for FSIMG in "$roroot_image" rorootfs.img rootfs.img ext3fs.img rootfs.tgz; do
...
[ -e "$FSIMG" ] && {
    ...
    [ "$FS" = unknown ] || {
        [ "$ro" ] && mount -m -n -t "$FS" -r "$FSIMG" /run/initramfs/rorootfs
        [ -b "$FSIMG" ] || {
            loopdev=$(losetup -f)
        ...
        if [ -d /run/initramfs/rorootfs/LiveOS ]; then
            [ -f /run/initramfs/rorootfs/LiveOS/rootfs.img ] && {
                FSIMG=/run/initramfs/rorootfs/LiveOS/rootfs.img
                det_fs "$FSIMG"
                load_fstype "$FS"
                BASE_LOOPDEV=$(losetup -f)
            }
        elif [ -d /run/initramfs/rorootfs/usr ] || [ -d /run/initramfs/rorootfs/ostree ]; then
...
case $cfg in
    ropt)
        install_Image
        mount -m -n -o ro "$FSIMG" /run/initramfs/rorootfs
        ;;
esac
...
[ "$FSIMG" ] && {
    [ "$writable_fsimg" ] && {
        ...
        case $FS in
            unknown) unpack_archive "$FSIMG" /run/initramfs/fsimg/ ;;
    ...
    [ "$live_ram" ] && [ ! "$OverlayFS" ] \
        || [ "$writable_fsimg" ] \
        || [ "$rd_overlay" = none ] || [ "$rd_overlay" = None ] || [ "$rd_overlay" = NONE ] && {
    ...
    [ -e "$loopdev" ] || {
        loopdev=$(losetup -f)
        losetup ${readonly_base:+-r} "$loopdev" "$FSIMG"
    }
    [ "$OverlayFS" ] || read -r sz < "/sys/class/block/${loopdev##*/}/size"
    if [ "$setup" = rw ]; then
        echo 0 "$sz" linear "$loopdev" 0 | dmsetup create live-rw
    else
...
if [ "$OverlayFS" ]; then
    if [ "$FSIMG" ]; then
        ...
        [ -f "$mount_unit" ] && echo "Type=$FS" >> "$mount_unit"
        if [ "${DRACUT_SYSTEMD-}" ]; then
            ln -sf "$FSIMG" /run/initramfs/rorootfs
            systemctl start run-rootfsbase.mount
        else
            srcPartition="$FSIMG" fstype="$FS" srcflags=,ro \
                mountPoint=/run/rootfsbase override=override \
