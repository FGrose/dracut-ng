...
getargbool 0 rd.overlayfs.readonly -d -y rd.live.overlay.readonly && readonly_overlay=--readonly
...
do_live_overlay() {
    ...
    elif strstr "$rd_live_overlay" ":"; then
        # pathspec specified, extract
        pathspec=${rd_live_overlay##*:}
    fi
    ...
    devspec=${rd_live_overlay%%:*}

    # need to know where to look for the overlay
    if [ -z "$setup" ] && [ -n "$devspec" ] && [ -n "$pathspec" ] && [ -n "$rd_live_overlay" ]; then
