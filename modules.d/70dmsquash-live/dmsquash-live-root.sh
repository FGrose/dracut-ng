...
>>>>>>> a1ead37f6 (refactor(partition-lib,overlayfs,btrfs): use partition-lib)
...
if [ -e "$FSIMG" ]; then
    ...
    [ "$ro" ] && mount -m -n -t "$FS" -r "$FSIMG" /run/rootfsbase
    ...
    if [ -d /run/rootfsbase/proc ]; then
        ...
        loopdev=$(losetup -f)
        losetup -r "$loopdev" /run/rootfsbase/LiveOS/rootfs.img
        FSIMG="$loopdev"
        det_fs "$FSIMG"
        load_fstype "$FS"
    fi
...
[ "$FSIMG" ] && {
    ...
    [ "$OverlayFS" ] || {
        ...
        read -r sz < "/sys/class/block/${loopdev##*/}/size"
    }
    if [ "$setup" = rw ]; then
