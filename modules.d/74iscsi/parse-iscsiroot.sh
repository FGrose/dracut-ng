...
if [ -n "$netroot" ] && [ "$root" != "/dev/root" ] && [ "$root" != "dhcp" ]; then
    if ! getargbool 1 rd.neednet > /dev/null || ! getarg "ip="; then
        /sbin/initqueue --unique --onetime --settled /sbin/iscsiroot dummy "'$(qescape "$netroot")'" "'$NEWROOT'"
    fi
fi
...
/sbin/initqueue --unique --onetime --timeout /sbin/iscsiroot timeout "'$(qescape "$netroot")'" "$NEWROOT"
