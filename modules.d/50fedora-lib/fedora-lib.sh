...
update_BootConfig() {
    ...
    umount "$root"/mnt
    /run/rootfsbase/usr/bin/sync -f /run/initramfs/ESP/"$ovl_dir"
...
\	menu_item 'Make a new, persistent overlay directory for the base image' '$_BOOTDIR' '' (\$esp)/'$_ovl_dir' '${rootcfg% rd\.ovl\.dir*}' 'rd.ovl.dir=PROMPT rd.overlay=UUID=$UUID,$_ovl_dir $cfgargs'\\
\	menu_item 'Format a new, persistence partition for the -$_ovl_dir- base image' '$_BOOTDIR' '' (\$esp)/'$_ovl_dir' '${rootcfg% rd\.ovl\.dir*}' 'rd.ovl.dir=PROMPT rd.overlay=$_ovl_dir,PROMPTSZ,PROMPTFS $cfgargs'\\
...
s;(serial=.*/)\S*;\1 rd\.overlay=UUID=$UUID ${ROOTFLAGS:+rootflags=$ROOTFLAGS };
}" "$GRUB_cfg"
    }
