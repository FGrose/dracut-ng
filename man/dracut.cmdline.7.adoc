...
+
The parameter with no or just the *_<fstype>_* argument specifies the
creation of a persistence partition for the overlay in adequate free space at
the end of the *root=*live:_<device specification>_ disk on its first boot.
Subsequent boots will ignore the fstype argument.
* *_<fstype>_* is the filesystem type to use for the overlay bearing partition.
