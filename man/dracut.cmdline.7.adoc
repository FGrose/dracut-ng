...
persistent overlay specified by *_rd.overlayfs=_*. +
...
and {`initrd.img|initrd`} files will be copied into the *rd.live.dir=_<path>_*
...
**rd.live.dir={**__<dirname>__|PROMPT}::
* *_<dirname>_* specifies the directory within the boot device where the
squashfs.img or rootfs.img can be found.  By default, this is `LiveOS`.
+
This directory name is also used as the parent directory name in the creation
of a persistent overlay directory.
+
The creation of a new OverlayFS overlay directory can be specified by including
a new **rd.live.dir=**__<dirname>__ on the kernel commandline.
...
**rd.overlay=**{[{__<size>__|PROMPTSZ}[,{__<fstype>__|PROMPTFS}[,new_pt_for:__<live_dir>__[,recreate[=__<devspec>__][,{__<devspec>__|PROMPTPT|PROMPTDK}[:{__<pathspec>__|auto}[,ea=__<extra_attr__,__s>__]]]]]]]|none}::
...
`/<rd.live.dir>/overlay-<label>-<uuid>`, where _<label>_ and _<uuid>_ are the
...
+
--
* The parameter with no or any of the filesystem related or *_<size>_*
...
+
* *_<size>_* specifies a partition size in gigabytes for the partition. If
a size is not provided, the partition will be made the size of the free space
at the end of the disk device. A minimum of 256 MiB of free space is required.
If *_<size>_* is larger than the free space, the free space value will be
used. If ''0'' is provided, no overlay will be provided.
...
+
* *_new_pt_for:<live_dir>_* specifies that a new persistent OverlayFS directory
will be created for the existing installation specified by *_<live_dir>_*.  The
...
the last arguments in the *_rd.live.overlay=_* list) that specifies extra make
