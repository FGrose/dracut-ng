...
**iso-scan/filename=**__<filepath>__::
    ...
    using _dmsquash-live/iso-scan.sh_ code and the *_iso-scan/filename_*
    parameter.
+
All mountable devices will be mounted readonly in `/dev/disk/by-uuid/*` order.
The first matching *_<filepath>_* is loop setup and mounted. +
+
NOTE: _ext[43]_, _f2fs_, & _xfs_ filesystems will be mounted with
`-o ro,noload` or `ro,norecovery` and kernel messages will be checked for a
dirty or hibernated filesytem, in which case the image will be checked with
`checkisomd5` for integrity.)
+
The parent disk device is always unmounted after the .iso image file has been
loop setup. +
A link to this disk is saved at `/run/initramfs/isoscandev`. +
A link to the image loop device is saved at `/run/initramfs/isoloop`.
+
...
menuentry 'Live .iso' --class gnu-linux --class gnu --class os {
    ...
    set isofile="/boot/isos/Live-x86_64.iso"
    loopback loop $isofile
    linux (loop)/boot/x86_64/loader/linux iso-scan/filename=$isofile root=live:LABEL=$isolabel ro rd.live.image quiet
