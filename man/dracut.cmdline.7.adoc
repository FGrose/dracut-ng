...
filesystem is not present at boot time, a tmpfs directory will be created at
`/run/overlayfs` in the `/run` tmpfs to provide temporary read-write storage.
This filesystem is typically sized to a default of one half of the physical RAM
in the system. One may use the *_rd.overlay=[,<size>]_* option to alter the
default; or the command: `mount -o remount,size=<nbytes> /run` may be used
to resize this virtual filesystem after booting. An entry like the following in
`/etc/fstab` will alter the default:
`none /run tmpfs defaults,size=<size|%> 0 0`
...
**rd.overlay=**{[1|__<source_name>__[,<size>[,__<devspec>__[:__<pathspec>__[,subvol=<subvol_path>]]]]]|0|no|off}::
...
+
Apply a temporary OverlayFS overlay to a standard block root partition by
entering simply **rd.overlay** on the kernel command line. The OverlayFS will
be mounted with the 'volatile' mode option avoiding all forms of sync calls to
the upper filesystem.
+
...
_os_rootfs_.
* *_<size>_* specifies the size of the tmpfs mounted for a virtual or transient
overlay. It may be specified as a percent of physical RAM with `[1-9]%` |
`[1-9][0-9]%`, or as a whole integer that will be interpreted as _<size>GiB_,
or it may be requested with units `1-9999G` or `100-9999M` for GiB and MiB.
...
_<partspec>_, where _<partspec>_ is a numeric only partition number or one of
the other _<devspec>_ specifications.
* *_PROMPTPT_* triggers the preparation of a console menu for selecting a
partition for the overlay.
* *_PROMPTDK_* likewise, a menu for selecting a disk device.
