...
menuentry 'Live .iso' --class gnu-linux --class gnu --class os {
  ...
  linux (loop)/boot/x86_64/loader/linux iso-scan/filename=$isofile root=live:/dev/loop0p1 ro quiet splash
...
overlays using OverlayFS.  This provides a default stateless boot where all
listed partitions are mounted transparently with overlays in a 'fsync=volatile'
manner.  It does not prevent accidental or intentional remounting persistence. 
