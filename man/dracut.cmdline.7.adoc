...
_btrfs.snapshot_
~~~~~~~~~~~~~~~~
Requires the dracut 'btrfs' & 'overlayfs' modules. The boot commandline
parameter _btrfs.snapshot_ triggers the creation of a `/.snapshots` btrfs
subvolume of the root filesystem and two snapshots named by the current
datetime. One is read-write and the second a read-only snapshot of the first
named `<datetime>-origin`.

A boot-time menu of available snapshots is presented to the user to select the
snapshot to launch.  If a read-only snapshot is selected, an OverlayFS mount
is used to overlay the read-only base snapshot with a transient or persistent
overlay specified by *_rd.overlayfs=_*. A transient overlay is the default.
...
`btrfs subvolume set-default 5 <rootfs mountpoint>` sets the default subvolume
to ID 5 (FS_TREE).
...
**rd.live.image**{[=__esp[=<sizeMiB>__]|[,{__<devspec>__|PROMPTDK}[,mklabel]]}[,ropt]::
...
* *ropt* signals the creation of a partition installed with the read-only root
filesystem (`squashfs.img` or `rorootfs.img` with SquashFS or EROFS image) from
a .iso image source.  This results in a LiveOS OverlayFS root filesytem with no
loop devices.
...
**rd.live.overlay=**{[{__<sizeGiB>__|PROMPTSZ}[,{__<fstype>__|PROMPTFS}[,new:__<live_dir>__[,recreate[=__<devspec>__][,__<devspec>__[:{__<pathspec>__|auto}[,{iso|ciso}[,ea=__<extra_attr__,__s>__]]]]]]]]|none}::
...
+
* *_new:<live_dir>_* specifies that a new persistent OverlayFS directory will
be created for the existing installation specified by *_<live_dir>_*.  The new
directory will be created in the partitions specified by *_<devspec>_*.  A new
persistence partition can be specified by omitting *_<devspec>_* or by using a
*PROMPT??* argument.
