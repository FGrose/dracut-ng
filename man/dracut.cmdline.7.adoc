...
Specify the base root partition as **root=ovl:__<path>__**.
+
...
undefined (see https://docs.kernel.org/filesystems/overlayfs.html#changes-to-underlying-filesystems).
For this reason, the persistent overlay will be discarded on a subsequent boot
if the base root device md5sum hash has changed. Note that just mounting the
base root filesystem read-write will cause the md5sum hash to change. Take note
also that checking a typically large base root filesystem partition may take
several minutes.
+
...
the **root=ovl:**__<path|url>__ device.
...
**root=**live:__<path to blockdevice>__ disk on its first boot.
+
