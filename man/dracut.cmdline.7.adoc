...
For live media images, see the **Booting live images** section below.
...
adding the optional arguments to this parameter.
+
...
the base root filesystem partition **root=**__<path to blockdevice>__ instead
of **root=**live:__<path to blockdevice>__ to indicate a live boot.
With argument
* *_esp[=<sizeMiB>]_* create a new EFI System Partition on the boot media with
the size specified.  The existing ESP will be replaced by a copy in the new
partition, and the source directory with the boot kernel and initrd files will
be copied into the **rd.live.dir=**__<path>__ directory of the new ESP.  These
files will be bind mounted onto their counterparts in the root filesystem /boot
directory to facilitate programmatic updates.  The default *_<sizeMiB>_* is
512 MiB for disks smaller than 32 GiB and another 256 MiB is added to the
default per 16 GiB of end-of-disk free space.
+
...
**root=**live:__<device specification>__ disk on its first boot. A distribution
specific library module should adjust the boot menu for subsequent boots.
...
* *_<devspec>_* specifies the path to a device with a mountable filesystem.
* *_<pathspec>_* is a path within the _<devspec>_ filesystem to either a file
(that is loop mounted for a Device-mapper overlay) or a directory (that is
symbolically linked to `/run/overlayfs` for a OverlayFS mount overlay). (A
...
code.
+
See the *OverlayFS* section introduction above.
