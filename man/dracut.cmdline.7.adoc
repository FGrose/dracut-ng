...
**rd.live.image**{[=__esp[=<sizeMiB>__]|[,{__<devspec>__|PROMPTDK}]}::
...
_<partspec>_, where _<partspec>_ is a numeric only partition number or one of
the other _<devspec>_ specifications.
* *PROMPTDK* triggers an interactive menu for selecting the target disk.
...
filesystem (squashfs.img or rorootfs.img with SquashFS or EROFS image) from a
.iso image source.  This results in a LiveOS OverlayFS root filesytem with no
loop devices.
