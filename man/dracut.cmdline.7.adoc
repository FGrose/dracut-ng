...
**rd.overlayfs=**{1|[__<devspec>__[:__<pathspec>__]]}::
...
**rd.live.overlay.overlayfs=**{[1|__<source_name>__]|0|no|off}::
...
*rd.live.overlay.overlayfs* is missing from the command line, it will be
...
*rd.live.overlay.overlayfs=_[1|<source_name>]_* is present when either the
...
parameter for consumption by the _overlayfs_ module code.
+
...
>>>>>>> f01e6adb (feat(overlayfs): decouple non-live OverlayFS boots from dmsquash-live)
ZIPL
~~~~
