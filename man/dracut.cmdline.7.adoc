...
**root=**__<device specification>__::
...
**rd.ovl.flags=**__<mount options>__::
    specify additional mount options for the partition bearing the OverlayFS
    overlay for a filesystem. If not set, _/etc/fstab_ will be parsed for
...
**rootfallback=**__<device specification>__::
...
Using the command `loopback <devicename> </path/to/imagefile>`, the GRUB
boot loader can make a device identified by `<devicename>` from an
image file on the boot or other specified device. This allows GRUB to load
the boot kernel and initramfs from the device made from the image file.

**iso-scan/filename=**[{__<devspec>__|PROMPTPT}:]{__<filepath>__|PROMPTDR=__<dirpath>__}::
    _dracut_ must independently identify the device holding the image file
    through its _dmsquash-live/iso-scan.sh_ code using the +
    *_iso-scan/filename_* parameter.
    * *_<devspec>_* may be specified as
    *{`block:|UUID=|PARTUUID=|LABEL=|PARTLABEL=|serial=<devserial>/serial/<partspec>`}*
    * *PROMPTPT* triggers an interactive menu for selecting the partition
    bearing the .iso file.
NOTE: Without the optional *_<devspec>_*, all mountable devices will be
searched in `/dev/disk/by-uuid/*` order for the file specified by
*_<filepath>_*.  The first matching file is set up as a loop device.
+
    * *_<filepath>_* specifies the image file for loop setup by _dracut_ to
    access the root filesystem within.
    * *PROMPTDR=_<dirpath>_* triggers an interactive menu for selecing a file
    from the _<directory path>_ within the selected partition's filesystem.

When either of the *PROMPT??* arguments are used, _kexec_ will be employed to
soft reboot the chosen image after its potentially older initramfs has been
concatenated with the necessary current dracut code and filesystem structure to
complete the requested boot.

NOTE: _kexec_ (kernel execute) may have trouble loading much older kernels due
to variation in memory loading. (Fedora 39 does successfully boot with _kexec
2.0.32_ found in Fedora 45.)  Tweeking the kexec options may or may not allow a
successful boot.

The disk device containing the .iso file will stay mounted after booting at
`/run/initramfs/isoscandev` unless *_rd.live.ram_* is requested, in which
case, the loop device and mount are detached so as to simplify installation to
the parent disk, if desired.

Using *_iso-scan/filename_* with a Fedora/Red Hat/CentOS Live iso should just
work by copying the original kernel cmdline parameters.

The GRUB boot loader will need its appropriate modules for the partition and
filesystem bearing the .iso file, such as __part_gpt__ or __part_msdos__ and
_ext2_, _btrfs_, _f2fs_, or _xfs_ and _iso9660_ for the iso filesystem.

In the first example below, the .iso image file is stored in the `/isos/`
directory of the boot partition so that GRUB can address the isofile for loop
setup to reach the linux kernel and initrd initramfs.

NOTE: The filepaths below to the linux kernel and initrd will differ depending
on the OS distribution and vintage. For example, Fedora 37-41 used paths
`/images/pxeboot/vmlinuz` & `/images/pxeboot/initrd.img`.

[listing]
.GRUB menuentry example employing the legacy all device scan.
--
menuentry 'Live .iso' --class gnu-linux --class gnu --class os {
  insmod part_gpt
  insmod ext2
  insmod iso9660
  insmod loopback
  set isofile=/isos/Live.x86_64.iso
  loopback loop "$isofile"
  linux (loop)/boot/x86_64/loader/linux iso-scan/filename=$isofile root=live:iso-scan ro quiet splash
  initrd (loop)/boot/x86_64/loader/initrd
}
--
In the following examples, the filesystem uuid of the storage partition
is set to the variable `dev_uuid`, enabling direct addressing of the isofile.
NOTE: This requires an initramfs built with an updated version of dracut.

[listing]
.grub.cfg script example for dynamic menuentry generation - requires GRUB module _regexp_ to be compiled in or available. Also accommodates multiple initrd images, if employed.
--
insmod part_gpt
insmod ext2
insmod iso9660
insmod regexp
insmod loopback
set dev_uuid=6c8468d0-b6fe-4bee-87a3-f5e38ef4f5fc
search --no-floppy --set dev -u $dev_uuid
for file in ($dev)/isos/*.iso; do
  regexp -s 2:isofile '^(\(.*\))?(\/isos\/.*)$' "$file"
  menuentry "Start ${isofile}" "${isofile}" "$dev" {
    isofile="$2"
    dev="$3"
    loopback loop ($dev)/isos/"$isofile"
    initrds=''
    for f in (loop)/boot/x86_64/loader/initrd*; do
      initrds="$initrds $f"
    done
    linux (loop)/boot/x86_64/loader/linux root=live:/dev/loop0p1 \
      iso-scan/filename=UUID=${dev_uuid}:/isos/"$isofile" ro quiet splash
    initrd $initrds
    loopback -d loop
  }
done
--
[listing]
.grub.cfg script example of dynamic menuentry generation for any `/isos/<isoname>.iso` on devices accessible at early boot. Requires GRUB modules _regexp_ and _probe_.
--
insmod part_gpt
insmod part_msdos
insmod ext2
...
insmod loopback
for dev in (*); do
  if [ -d $dev/isos -a -f $dev/isos/*.iso ]; then
    probe --set dev_uuid --fs-uuid "$dev"
    for file in ($dev)/isos/*.iso; do
      regexp -s 2:isofile '^(\(.*\))?\/isos\/(.*)$' "$file"
      if ! loopback loop "$file"; then continue; fi
      if [ -d (loop)/LiveOS ]; then
        menuentry "Start a transient ${isofile}" "$dev" "$dev_uuid" "${isofile}" {
        dev="$2"
        dev_uuid="$3"
        isofile="$4"
        loopback loop ($dev)/isos/"$isofile"
...
**rd.info**::
    Print some informational output even if *quiet* is set on the kernel command
    line.

**rd.debug**::
*    If _systemd_ is active in the initramfs,
        all output is logged to the _systemd_ journal, which you can inspect
        with `journalctl -ab`.
*    If _systemd_ is not active,
        the logs are written to _dmesg_ and _/run/initramfs/init.log_.
        If *quiet* is set on the kernel command line, it also logs to the
        console.

**quiet**::
    Disable most log messages, except as noted above.

*_rd.info_*, *_rd.debug_*, & *_quiet_* are queried through the library function
_cfg_output()_, which also exports global variables VERBOSE, QUIET, OUTPUT, &
SHOW_INFO used to control logging and console output.
...
_rd.btrfs.snapshot_
~~~~~~~~~~~~~~~~~~~
**rd.btrfs.snapshot**[=__<source_name>[,{ro|rw}:<subv_path>]]__::
Requires the dracut 'btrfs' & 'overlayfs' modules. The boot command line
parameter _rd.btrfs.snapshot_ without additional arguments triggers the
creation of a `/.snapshots` btrfs subvolume of the root filesystem and two
snapshots named by the current local datetime. One is read-write and the second
a read-only snapshot of the first named `<datetime>-origin`. Then a boot-time
menu of available snapshots is presented to the user to select the snapshot to
launch.  If a read-only snapshot is selected, an OverlayFS mount is used to
overlay the read-only base snapshot with either a transient (the default) or a
persistent overlay specified by *_rd.overlay=_*. +
* *_=<source_name>_* optionally sets the OverlayFS mount source name on a
persistent overlay of a read-only snapshot. The default is os_snapfs.
* *_{ro|rw}:<subv_path>_* specifies creation of a new ro|rw snapshot at
`/.snapshots/<subv_path>`. (Existing btrfs snapshots may be specified for
booting using the *_rootflags={subvolid|subvol}=<subv_spec>_* kernel command
line option.)

The command `btrfs subvolume set-default <snapshot_volid> <rootfs mountpoint>`
is used to control the subvolume to mount.  Should there be any failure,
`btrfs subvolume get-default <rootfs mountpoint>` may help diagnose the state. +
`btrfs subvolume set-default 5 <rootfs mountpoint>` restores the default
subvolume to `ID 5 (FS_TREE)`.

OverlayFS
~~~~~~~~~
**root=ovl:**__<device specification>__::
    specify the block device partition with a root filesystem that will be
    mounted read-only and serve as the base for an OverlayFS mount.

Requires the dracut 'overlayfs' module that allows a base filesystem to be
mounted read-only and have changes recorded, either temporarily or persistently
in an OverlayFS overlay directory.

OverlayFS overlays are directories of the files that have changed on a
read-only base (lower) filesystem.  The full filesystem is provided through a
special overlay type mount that merges at least two directories, designated the
lower and the upper.

If an OverlayFS upper directory for a base root filesystem is not present at
boot time, a tmpfs will be mounted at `/run/ovl` and a directory created at
`/run/ovl/upperdir` to provide temporary read-write storage.  The OverlayFS
will be mounted using the 'volatile' mode option, which avoids all forms of
sync calls to the upper filesystem.
...
available root filesystem storage up to the capacity of the designated
persistence disk device.

For live media images, see the link:https://github.com/dracut-ng/dracut-ng/edit/main/man/dracut.cmdline.7.adoc#booting-live-images[*Booting live images*] section below.

This filesystem is typically sized to a default of one half of the physical RAM
in the system.  This size may be altered post boot with the command:
`mount -o remount,size=<nbytes> /run/ovl`. An entry like the following in
`/etc/fstab` may also be used: `none /run/ovl tmpfs defaults,size=<size|%> 0 0`
...
directory.  This allows controlling the size of the writable overlay at boot
time, which is useful for deployments where the available RAM is known in
advance, e.g., MAAS ephemeral environments.
+
If the size greater than the physical RAM, the tmpfs will begin to use
available swap space with its slower disk speeds once the available RAM is
exhausted.
+
...
**rd.overlay=**{[1|__<source_name>__]|[,__<devspec>__[:__<pathspec>__]][,subvol=<subvol_path>]|0|no|off}::
Enables the use of the *OverlayFS* kernel module ('overlay'), if available,
to provide a copy-on-write union directory for the root filesystem.
+
One may apply a temporary OverlayFS overlay to a standard block root partition
by entering simply **rd.overlay** on the kernel command line, or use the
*rd.overlay=*_tmpfs:[size=<size>[,nr_blocks=<n>[,nr_inodes=<n>]]]_ options to
specify the tmpfs mount options.  
+
Apply a persistent OverlayFS overlay to a standard block root partition by
adding the _<devspec>:<pathspec>_ arguments to this parameter.
+
IMPORTANT: If changes to the base root filesystem are made when the OverlayFS
is unmounted or offline, subsequent behavior of the mounted overlay may be
undefined if any of the “metacopy”, “index”, “xino” and “redirect_dir” features
have been used. Avoid these features in the OverlayFS mount. (See
https://docs.kernel.org/filesystems/overlayfs.html#changes-to-underlying-filesystems.)

* {[1|*_<source_name>_*]} specifies an OverlayFS overlay, optionally setting
the OverlayFS mount source name to _<source_name>_ instead of the default
_os_rootfs_.
* *_<devspec>_* specifies the path to a device with a mountable filesystem.
*_<devspec>_* may be in the form of
+
{`serial=<devserial>/serial/<partspec>|UUID=?*|PARTUUID=?*|LABEL=?*|PARTLABEL=?*|/dev/?*`}
+
serial=_<devserial>_/serial/_<partspec>_ specifies the ID_SERIAL_SHORT
property value for the target disk device followed by `/serial/` and
_<partspec>_, where _<partspec>_ is a numeric only partition number or one of
the other _<devspec>_ specifications.
* *_<pathspec>_* is a path within the *_<devspec>_* filesystem to a directory,
which is symbolically linked to `/run/overlayfs` for a OverlayFS mount overlay.
(A required sister directory `/<pathspec>/../ovlwork` is automatically made.)
+
The default *_pathspec_*, is `/<rd.ovl.dir>/overlay-<label>-<uuid>`, where
_<label>_ and _<uuid>_ are the LABEL and UUID of the filesystem specified by
the **root=ovl:**__<device spec|url>__ specification.
+
* *_subvol=<subvol_path>_* optionally provides a btrfs subvolume path on
_<devspec>_ that will be added to the mount options for that device.
...
**rd.ovl.dir=**__<dirname>__::
Specifies the directory within the persistence device where the OverlayFS
upper directory can be found.  By default, this is `RootOvl`.
+
The creation of a new OverlayFS overlay can be specified by including a new
**rd.ovl.dir=**__<dirname>__ on the kernel commandline.

**rd.overlay.readonly=**1::
This is used to boot in a read-only mode with a normally read-write persistent
overlay.  An OverlayFS mount with this option will link the persistent overlay
directory to `/run/overlayfs‑r` as an additional read-only lower layer stacked
under `/run/overlayfs`, the temporary, writable, upper directory overlay and
over the base root filesystem to complete the bootable root filesystem.  This
parameter will trigger mounting the OverlayFS with the 'volatile' mode option.

**rd.overlay.all=**1::
This is used to boot all non-pseudo/swap filesystems as readonly with tmpfs
overlays using OverlayFS (excepting those not suitable for OverlayFS, such as
vfat).  This provides a default stateless boot where all listed partitions are
mounted transparently with overlays in a 'fsync=volatile' manner.  It does not
prevent accidental or intentional remounting persistence.
...
Booting live images
~~~~~~~~~~~~~~~~~~~
**root=live:**__<device specification>__::
    specify the block device partition with a root filesystem that will be
    mounted read-only and serve as the base for an OverlayFS mount or
    Device-mapper snapshot.

Requires the dracut 'dmsquash-live' module.
Dracut offers multiple options for live booted images through the
_dmsquash-live_ and _overlayfs_ modules augmented by _dmsquash-live-ntfs_,
_livenet_, _squash_, & _partition-lib_ among other modules.

SquashFS or EROFS (read-only) base filesystem image:::
Note -- There are 3 separate overlay types available:
- Device-mapper snapshots (the original offering),
- Device-mapper thin provisioning snapshots (see _rd.overlay.thin_,
...
the base, read-only SquashFS or EROFS root filesystem. These methods enable a
relatively fast boot and lower RAM usage.
...
When _rd.overlay=_ is not specified for persistent overlay storage, or the
specified file is not found or writable, a Device-mapper snapshot based
...
of 32 GiB in RAM, and can be adjusted with the _rd.overlay.size=_ kernel
...
The internal SquashFS or EROFS structure is traditionally expected to be:

[listing]
----
squashfs.img          |  SquashFS or EROFS from LiveCD .iso
   !(mount)
   /LiveOS
...
For OverlayFS mount overlays, the internal SquashFS or EROFS structure may be a
direct compression of the root filesystem:
...
**root=**live:__<device specification>__ or **root=**live:__<URL>__ to specify
the location of your squashed root filesystem.

- The compressed SquashFS or EROFS image can be copied during boot to RAM at
`/run/initramfs/rorootfs.img` by using the **rd.live.ram=1** option.
- A device with a persistent overlay can be booted read-only by using the
**rd.overlay.readonly** option on the kernel command line.  This will
...
**rd.live.image**{[=__esp[=<sizeMiB>__]|[,{__<devspec>__|PROMPTDK}[,{ropt|iso|ciso}[,mklabel]]]}::
This parameter without an argument supports the legacy syntax of passing just
the base root filesystem partition *_root=<device specification>_* instead
of *_root=live:<device specification>_*.
With argument
* *_esp[=<sizeMiB>]_* a new EFI System Partition will be created on the boot
media with the size specified.  The existing ESP will be replaced by a copy in
a new partition, and the image directory with the boot kernel {`vmlinux|linux`}
and {`initrd.img|initrd`} files will be copied into the *rd.ovl.dir=_<path>_*
directory of the new ESP.  These files will be bind mounted onto their
counterparts in the root filesystem /boot/ directory to facilitate programmatic
updates.  The default *_<sizeMiB>_* is 512 MiB for disks smaller than 32 GiB
and another 256 MiB is added to the default per 16 GiB of end-of-disk free
space.
+
NOTE: When a target disk is partitioned by a **rd.overlay=** directive,
a new, default ESP will be created automatically and the esp parameter can be
omitted.
+
This feature facilitates updating the image kernel version and installing
multiple boot images or alternative initrd images on the boot device.
* *_<devspec>_* specifies the path to a device with a mountable filesystem and
is used to designate the target disk when a new installation is desired on an
arbitrary disk.
*_<devspec>_* may be in the form of
+
{`serial=<devserial>/serial/[<partspec>]|UUID=?*|PARTUUID=?*|LABEL=?*|PARTLABEL=?*|/dev/?*`}
+
serial=_<devserial>_/serial/[_<partspec>_] specifies the ID_SERIAL_SHORT
property value for the target disk device followed by `/serial/` and optional
_<partspec>_, where _<partspec>_ is a numeric only partition number or one of
the other _<devspec>_ specifications.
* *PROMPTDK* triggers an interactive menu for selecting the target disk where a
new persistence partition will be created.
* *_ropt_* signals the creation of a readonly partition for the OS root
filesystem. The EROFS or SquashFS root filesystem is copied into the new
partition usually from an isofile or iso partition.
* *_iso_* specifies configuration of an *_iso-scan/filename_* image file for
booting with persistence in the *rd.overlay=* designated overlay.  The boot
kernel and initramfs will be copied to the boot device EFI System Partition and
the GRUB menu will be updated with this boot target.
* *_ciso_* specifies that the *_iso-scan/filename_* image file is to be copied
to the `/isos/` directory of the *rd.overlay=* designated overlay and used as a
new boot target on the disk. As above, the boot device EFI System Partition and
the GRUB menu will be updated for this boot target. 
* *mklabel* causes the specified disk to be totally repartitioned with a new
GUID Partition Table (GPT). A new EFI System Partition of default size will be
created.  One of the installation options *{_ropt|iso|ciso_}* must be included.
...
or EROFS is involved during boot.
+
...
of your compressed filesystem image tarball, SquashFS, or EROFS image.
...
**rd.live.check**::
Requests a CD/DVD/USB media check with checkisomd5.

**rd.ovl.dir={**__<dirname>__|PROMPT}::
* *_<dirname>_* specifies the directory within the live device where changes
to the base root filesystem can be persisted.  This was previously called
*rd.live.dir*.  In a LiveUSB installation, this is where the root filesystem
image, squashfs.img or rootfs.img, can be found.  By default, this is `LiveOS`.
+
The creation of a new OverlayFS overlay directory can be specified by including
a new *rd.ovl.dir=_<dirname>_* on the kernel commandline.
* *PROMPT* may be entered to invoke an interactive dialog to specify the
directory name for a new OverlayFS directory.

**rd.live.rorootimg=**__<filename of SquashFS or EROFS image>__::
Specifies the filename for a read-only image of the root filesystem.
By default, this is _squashfs.img_.

**rd.live.squashimg=**__<filename of SquashFS or EROFS image>__::
Specifies the filename for a SquashFS or EROFS image of the root filesystem.
_Deprecated_ use rd.live.rorootimg instead.
By default, this is __squashfs.img__.

**rd.live.ram=**1::
Copy the complete image to RAM and use this for booting. This is useful
when the image resides on, _e.g._, a DVD which needs to be ejected later on.

**rd.overlay=**{[{__<size>__|PROMPTSZ}[,{__<fstype>__|PROMPTFS}[,new_pt_for:__<ovl_dir>__[,recreate[=__<devspec>__][,{__<devspec>__|PROMPTPT|PROMPTDK}[:{__<pathspec>__|auto}[,ea=__<extra_attr__,__s>__]]]]]]]|none}::
Manage the specification of a persistent overlay that can be used to save
changes made to the root filesystem within the +
*root=*live:_<device specification>_ or *root=*live:_<url>_ device.
+
The default _pathspec_, when *:auto* or no **:**__<pathspec>__ is given, is
`/<rd.ovl.dir>/overlay-<label>-<uuid>`, where _<label>_ and _<uuid>_ are the
LABEL and UUID of the filesystem specified by the
**root=**live:__<path|url>__ device.
+
If a persistent overlay __is detected__ at the standard LiveOS path, and
_rd.overlay_ is not set to 1, the overlay type (either Device-mapper or
OverlayFS) will be detected and it will be used.
+
--
* The parameter alone or with any of the filesystem related or *_<size>_*
arguments specifies the creation of a persistence partition named 
__LiveOS_persist__ for the overlay in adequate free space at the end of the
*root=*live:_<device specification>_ disk on its first boot.
+
NOTE: The operating system distribution should supply a _dracut_
<distribution>-live-lib module to edit the boot loader menu so that subsequent
 boots will ignore any filesystem related or partition size arguments.
+
* *_<size>_* specifies a partition size as a whole integer that will be
interpreted as _<size>GiB_, or it may be requested with units `1-9999G` or
`100-9999M` for GiB and MiB. If a size is not provided, the partition will be
made the size of the free space at the end of the disk device. A minimum of
256 MiB of free space is required. If *_<size>_* is larger than the free space,
the free space value will be used. If ''0'' is provided, no overlay will be
provided.
* *PROMPTSZ* may be entered to invoke an interactive dialog to specify the
partition size in whole gigabytes.
* *_<fstype>_* is the filesystem type to use for the persistence partition.
*_<fstype>_* for a new partition may be [*btrfs*|*ext*[*432*]|*f2fs*|*xfs*];
the default is *ext4* for maximum availability, although *f2fs* or *btrfs* may
be preferred for greater endurance on common flash storage devices.
* *PROMPTFS* may be entered to invoke a menu to select the filesystem type.
+
NOTE: In an *_iso-scan/filename_* boot, if any of the above arguments are
supplied without a *_<devspec>_* argument (such as in a mklabel boot), a comma—
`,`—should precede or follow all these arguments to indicate that *_<devspec>_*
is specified elsewhere.
+
* *_new_pt_for:<ovl_dir>_* specifies that a new persistent OverlayFS directory
will be created for the existing installation specified by *_<ovl_dir>_*.  The
new directory will be created in the partitions specified by *_<devspec>_*.  A
new persistence partition can be specified by omitting *_<devspec>_* or by
using a *PROMPT??* argument.
* *_recreate_*[=_<devspec>_] specify the destruction and recreation of a
partition no larger than the contiguous free space at the one destroyed. If no
_<devspec>_ is provided, the partition named __LiveOS_persist__ on the root
bearing disk will be targeted. Useful for changing the partition size or
filesystem type or attributes. *All* saved content in the partition is lost.
Compare: **rd.overlay.reset** will only destroy the OverlayFS upper
directory and then recreate an empty replacement.
* *_<devspec>_* specifies the path to a device with a mountable filesystem.
*_<devspec>_* may be in the form of
+
{`serial=<devserial>/serial/<partspec>|UUID=?*|PARTUUID=?*|LABEL=?*|PARTLABEL=?*|/dev/?*`}
...
_<partspec>_, where _<partspec>_ is a numeric only partition number or one of
the other _<devspec>_ specifications.
* *PROMPTPT* triggers the preparation of a console menu for selecting a
partition for the overlay.
* *PROMPTDK* likewise, a menu for selecting a disk device.
...
automatically made.)
* *_ea=<extra_attr,s>_* in a comma-separated string (which, if present, must be
the last arguments in the *_rd.overlay=_* list) that specifies extra make
filesystem attributes or options particular to the *_<fstype>_*. See the
mkfs_config() function in fs-lib.sh to learn how the extra attributes are
applied.
...
**root=**live:__<device specification>__ or **root=**live:__<url>__ device.
...
by the **root=**live:__<path|url>__ device.  A new OverlayFS overlay for the
base root filesystem will be generated by specifying a new
**rd.live.dir=**__<dirname>__.
...
**rd.overlay=**{[1|__<source_name>__]|0|no|off}::
Requires the dracut _overlayfs_ module. (*rd.live.overlay.overlayfs* is the
legacy symbol for this option.)
+
Enables the use of the *OverlayFS* kernel module (overlay), if available, to
provide a copy-on-write union directory for the root filesystem.   Optionally,
sets the OverlayFS mount source name to _<source_name>_ instead of the default
_LiveOS_rootfs_.  On non-live systems _os_rootfs_ is the default
_<source_name>_.  On LiveOS systems that require an OverlayFS overlay, if
*rd.overlay* is missing from the command line, it will be
added to `/etc/kernel/cmdline` as _LiveOS_rootfs_, but having this option on
the command line avoids a daemon-reload on DRACUT_SYSTEMD based boots.  If
*rd.overlay=_[1|<source_name>]_* is present when either the
OverlayFS module is unavailable or the persistent overlay is a Device-mapper
snapshot type, then the parameter will be set automatically to `0` to inhibit
OverlayFS configuration. The _<source_name>_ is written to the *rd.overlay=*
parameter in `/etc/kernel/cmdline` for consumption by the _overlayfs_ module
code.
+
See the link:https://github.com/dracut-ng/dracut-ng/blob/main/man/dracut.cmdline.7.adoc#overlayfs[*OverlayFS*] section introduction above.
...
of the base root filesystem and the persistent overlay,
* writable `rootfs.img` images will use the above over a read-only loop device,
* OverlayFS mounts will link the persistent overlay directory at
...
directory overlay, to complete the bootable root filesystem. This parameter
will trigger mounting an OverlayFS with the 'volatile' mode option. The dracut
'overlayfs' module is required here.

**rd.overlay.size=**__<size_MiB>__::
...
**rd.zipl=**__<device specification>__::
...
_/run/initramfs/cmdline.d/*.conf_::
    Read by getcmdline().  Not used in dracut, but potentially by 3rd parties.

_/etc/kernel/cmdline_::
    Used for boot-time changes to dracut-processed command line options
    overriding others—including those on the kernel command line.
    Advantage: its read by systemd.
+
[listing]
.Example of changing an option at boot time:
--
printf '%s' " rd.overlay=0" >> /etc/kernel/cmdline
--
