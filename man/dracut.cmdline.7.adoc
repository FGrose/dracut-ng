...
Using the GRUB command `loopback <devicename> </path/to/imagefile>`, the GRUB
boot loader can make a disk device identified by `<devicename>` from an
...
_dracut_ must independently identify the disk device holding the image file
using _dmsquash-live/iso-scan.sh_ code and the *_iso-scan/devspec_* parameter
or by scanning all discs for the *_iso-scan/filename_* parameter argument.

**[iso-scan/devspec=**__<devspec>__]::
    * *_<devspec>_* may be specified as +
    *{{`[block:]|UUID=|PARTUUID=|LABEL=|PARTLABEL=}<appropriate id>|SERIALID=<id_serial_short>/SERIALID/[<partspec>]`}*

**iso-scan/filename=**__<filepath>__::
    * *_<filepath>_* specifies the image file for loop setup and mounting by
    _dracut_ to access the root filesystem within. +
+
NOTE: _Without_ the optional *_iso-scan/devspec_* parameter, all mountable
devices will be mounted readonly in `/dev/disk/by-uuid/*` order. +
(_ext[43]_, _f2fs_, & _xfs_ filesystems will be mounted with `-o ro,noload` or
`ro,norecovery`.) +
The first matching *_<filepath>_* that is successfully loop setup and mounted
is booted.
+
Kernel messages from mounting will be checked for a dirty or hibernated
filesytem, in which case the image will be checked with `checkisomd5` for
integrity.
+
The parent disk device is always unmounted after the .iso image's loop device
has been mounted. +
A link to this disk is saved at `/run/initramfs/isoscandev`. +
A link to the image loop device primary partition is saved at
`/run/initramfs/isoloop`.
+
...
+
IMPORTANT: In the example below, the .iso image file is stored in a directory
of the *boot partition* (`/isos/`) so that GRUB can address the isofile for
loop setup and reach the linux kernel and initrd initramfs.
+
NOTE: The filepaths to the linux kernel and initrd will differ depending
on the OS distribution and vintage. For example, Fedora 37-41 used paths
`/images/pxeboot/vmlinuz` & `/images/pxeboot/initrd.img` and earlier versions
used `isolinux/vmlinuz0` & `isolinux/initrd0.img`.
+
[listing]
...
menuentry 'Live .iso' --class gnu-linux --class gnu --class os {
  set isopath=isos/Live.x86_64.iso
  loopback loop "$isopath"
  linux (loop)/boot/x86_64/loader/linux root=live:iso-scan ro quiet splash \
    iso-scan/filename="$isopath"
  initrd (loop)/boot/x86_64/loader/initrd
}
--
In the following example, the filesystem uuid of a storage partition is set to
the variable `dev_uuid`, enabling direct addressing of an isofile on that
filesystem.  On btrfs filesystems, the default root subvolume may need to be
included as an additional directory level for GRUB as indicated by
<btrfs_root_subvol/> in the example below.
+
NOTE: This requires an initramfs built with an updated version of dracut.
+
[listing]
.grub.cfg script example for direct isofile access. Also accommodates multiple initrd images, if employed.
--
insmod part_gpt
insmod ext2
insmod iso9660
insmod loopback
menuentry "Start 'Live .iso'" --class gnu-linux --class gnu --class os {
  set dev_uuid=d746fe81-d206-4cc8-8da9-2130399dbf8f
  search --no-floppy --set dev -u $dev_uuid
  set isopath=isos/Live.x86_64.iso
  loopback loop ($dev)/<btrfs_root_subvol/>"$isopath"
  initrds=''
  for f in (loop)/boot/x86_64/loader/initrd*; do
    initrds="$initrds $f"
  done
  linux (loop)/boot/x86_64/loader/linux root=live:iso-scan ro quiet splash \
    iso-scan/devspec=UUID=${dev_uuid} iso-scan/filename="$isopath"
  initrd $initrds
}
--
...
rd.live.debug:: rd.debug (since version 113)
