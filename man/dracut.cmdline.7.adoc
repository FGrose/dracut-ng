...
**rd.overlayfs=**{[1|__<source_name>__]|[,__<devspec>__[:__<pathspec>__]][,subvol=<subvol_path>]|0|no|off}::
...
entering simply **rd.overlayfs** on the kernel command line. The OverlayFS will
...
**rd.overlayfs.readonly** option on the kernel command line.  This will
...
**rd.live.overlay=**{[{__<sizeGiB>__|PROMPTSZ}[,{__<fstype>__|PROMPTFS}[,new_pt_for:__<ovl_dir>__[,recreate[=__<devspec>__][,{__<devspec>__|PROMPTPT|PROMPTDK}[:{__<pathspec>__|auto}[,{iso|ciso}[,ea=__<extra_attr__,__s>__]]]]]]]]|none}::
...
_rd.overlayfs_ is not set to 1, the overlay type (either Device-mapper or
...
Compare: **rd.overlayfs.reset** will only destroy the OverlayFS upper
...
rd.live.overlay
rd.live.overlay=f2fs
--

**rd.overlayfs=**{[1|__<source_name>__]|0|no|off}::
...
*rd.overlayfs* is missing from the command line, it will be
...
*rd.overlayfs=_[1|<source_name>]_* is present when either the
...
OverlayFS configuration. The _<source_name>_ is written to the *rd.overlayfs=*
...
rd.live.overlay.reset:: rd.overlayfs.reset
...
--
printf '%s' " rd.overlayfs=0" >> /etc/kernel/cmdline
--
