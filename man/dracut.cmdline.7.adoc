...
lower and the upper.  If an OverlayFS upper directory for a base root
filesystem is not present at boot time, a tmpfs will be mounted at `/run/ovl`
and a directory created at `/run/ovl/upperdir` to provide temporary read-write
storage.  Persistent storage can be provided on vfat or msdos formatted devices
by supplying the OverlayFS upper directory within an embedded filesystem that
...
specify the tmpfs mount options.  The OverlayFS will be mounted using the
'volatile' mode option, which avoids all forms of sync calls to the upper
filesystem.
+
...
Non-persistent OverlayFS mounts will have the 'volatile' mode option applied at
mount time. This avoids all forms of sync calls to the upper filesystem.
