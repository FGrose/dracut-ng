...
**rd.ovl.flags=**__<mount options>__::
    specify additional mount options for the partition bearing the root
    filesystem OverlayFS overlay. If not set, _/etc/fstab_ will be parsed for
...
directory overlay, to complete the bootable root filesystem. Requires the
dracut 'overlayfs' module.
