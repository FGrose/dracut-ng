...
**rd.live.image**{[=__esp[=<sizeMiB>__]|[,{__<devspec>__|PROMPTDK}[,{iso|ciso}[,mklabel]]]}::
This parameter without an argument supports the legacy syntax of passing just
the base root filesystem partition *root=_<device specification>_* instead
of *root=live:_<device specification>_*.
With argument
...
booting with persistence in the designated overlay.  The boot kernel and
initramfs will be copied to the boot device EFI System Partition and the GRUB
menu will be updated with this boot target.
* *_ciso_* specifies that the *_iso-scan/filename_* image file is to be copied
to the `/isos/` directory of the just designated overlay and used as a new boot
target on the disk. As above, the boot device EFI System Partition and the GRUB
menu will be updated for this boot target. 
...
**root=**live:__<device specification>__ disk on its first boot.
+
